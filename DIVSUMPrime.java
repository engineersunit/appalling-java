  ...
  public static void main(String[] args) throws NumberFormatException, IOException {
    ...
    for (int j = 0; j < numOfTestCases; j++) {
      ...
      if(Arrays.binarySearch(primeNums, testCaseData[j])>-1){
        testCaseResult[j] = testCaseData[j] + 1;
        continue;
      }

      int sumOfPrimeFactors = 1;
      int sqrtTestCaseData =((Double)Math.ceil(Math.sqrt(testCaseData[j]))).intValue();
      for (int i = 0; i < primeNums.length && primeNums[i] <= sqrtTestCaseData; i++) {
        if (primeNums[i] == 0) {
          break;
        }
        while (testCaseData[j] % primeNums[i] == 0) {
